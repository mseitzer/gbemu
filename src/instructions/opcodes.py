...
def add_ind_cycles(op, cycles, dest = None, src = None):
    if dest is None:
        instr = "{0} {{ dest: _, src: Addr::{1} }}".format(op, src)
    else:
        instr = "{0} {{ dest: Addr::{1}, src: _ }}".format(op, dest)

    instr_list = instr_cycles.setdefault(cycles, [])
    if not (instr, False) in instr_list: instr_list.append((instr, False))
...
for code in range(0, 256):
    ...
    elif lo == 2 and 0 <= hi <= 3:
        op = "st8_ind"
        dest = [bc, de, hli, hld][hi]
        dtype = addr
        src = a
        add_ind_cycles(op, 2, dest=dest)
    ...
    elif lo == 0xA and 0 <= hi <= 3:
        op = "ld8_ind"
        src = [bc, de, hli, hld][hi]
        stype = addr
        dest = a
        add_ind_cycles(op, 2, src=src)
    ...
    elif lo == 0xA and hi == 0xE:
        op = "st8_ind"
        dest = IMM
        dtype = addr
        src = a
        imm = imm16
        add_ind_cycles(op, 4, dest=dest)

    elif lo == 0xA and hi == 0xF:
        op = "ld8_ind"
        src = IMM
        stype = addr
        dest = a
        imm = imm16
        add_ind_cycles(op, 4, src=src)
    ...
    if op != "":
        ...
        if dest != "" or src != "":
            out += " {"
            if dest != "":
                out += " dest: {0}::{1}".format(dtype, dest)
                if src != "":
                    out += ", src: {0}::{1}".format(stype, src)
